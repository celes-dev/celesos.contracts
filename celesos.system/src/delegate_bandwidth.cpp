...
void system_contract::sellram(name account, int64_t bytes)
{
    ...
    if (fee > 0)
    {
        ...
        (
            ...
            {account, ramfee_account, eosio::asset(fee, core_symbol()), std::string("sell ram fee")});
    }
}
...
/**
     ...
     * ram随时间衰减函数
     */
void system_contract::ramattenuator(name account)
{
    user_resources_table userres(_self, _self.value);
    auto item = userres.lower_bound(account.value);
    ...
    int64_t bytes = ram_attenuation(account.value);
    ...
    (token_account, {ram_account, active_permission},
     {ram_account, ramfee_account, tokens_out, std::string("ram fee")});

    set_resource_limits(item->owner.value, item->ram_bytes, item->net_weight.amount, item->cpu_weight.amount);
}
...
void system_contract::ramattenuator()
{
    ...
    _gstate.last_account = item->owner.value;
}
                               ...
                               const asset stake_net_delta, const asset stake_cpu_delta, bool transfer)
{
    ...
    eosio_assert(std::abs((stake_net_delta + stake_cpu_delta).amount) >= std::max(std::abs(stake_net_delta.amount), std::abs(stake_cpu_delta.amount)),
