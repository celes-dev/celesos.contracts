...
namespace celesossystem {
   ...
   :native(s,code,ds),
    ...
    _dbps(get_self(), get_self().value),
    _dbpunishs(get_self(), get_self().value),
    _burninfos(get_self(), get_self().value),
    _burnproducerstatinfos(get_self(), get_self().value),
    _burnblockstatinfos(get_self(), get_self().value),
   ...
   void system_contract::setnamelist(uint8_t list_type, uint8_t action_type, const std::vector<eosio::name>& namelist)
   {
      ...
      require_auth(get_self());
      eosio::check(list_type > 0 && list_type < enum_list_type::enum_unknown_list_type, "unknow list type.(1:actor_blacklist 2:contract_blacklist 3:resource_greylist)");
      eosio::check(action_type > 0 && action_type < enum_action_type::enum_unknown_action_type, "unknow action type.(1:insert 2:remove)");

      auto packed_names = pack(namelist);
      eosio::set_name_list_packed(static_cast<uint32_t>(list_type), static_cast<uint32_t>(action_type), packed_names.data(), packed_names.size());
   }

   void native::linkauth(ignore<name> account,
                        ignore<name> code,
                        ignore<name> type,
   ...
   void system_contract::init( unsigned_int version, const symbol& core ) {
      ...
      auto system_token_supply   = celes::token::get_supply(token_account, core.code() );
