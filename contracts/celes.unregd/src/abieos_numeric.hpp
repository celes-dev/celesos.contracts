...
namespace abieos {
    ...
    std::array <uint8_t, size> base58_to_binary(std::string_view s) {
        ...
        for (auto &src_digit : s) {
            ...
            if (carry < 0) eosio_assert(0, "invalid base-58 value");
            for (auto &result_byte : result) {
                int x = result_byte * 58 + carry;
                result_byte = x;
                carry = x >> 8;
            }
            if (carry) eosio_assert(0, "base-58 value is out of range");
        }
    ...
    public_key string_to_public_key(std::string_view s) {
        if (s.size() >= 5 && s.substr(0, 5) == "CELES") {
            auto whole = base58_to_binary<37>(s.substr(5));
