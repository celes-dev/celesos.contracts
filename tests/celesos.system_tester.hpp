...
public:
   ...
   void create_core_token( symbol core_symbol = symbol{CORE_SYM} ) {
      ...
      create_currency( N(celes.token), config::system_account_name, asset(100000000000000, core_symbol) );
   ...
   void remaining_setup() {
      ...
      BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"), get_balance("eosio")  + get_balance("celes.ramfee") + get_balance("celes.stake") + get_balance("celes.ram") );
   }
   ...
   asset get_balance( const account_name& act, symbol balance_symbol = symbol{CORE_SYM} ) {
      vector<char> data = get_row_by_account( N(celes.token), act, N(accounts), balance_symbol.to_symbol_code().value );
   ...
   abi_serializer initialize_multisig() {
      abi_serializer msig_abi_ser;
      {
         ...
         BOOST_REQUIRE_EQUAL( success(), buyram( "eosio", "celes.msig", core_sym::from_string("5000.0000") ) );
         produce_block();
