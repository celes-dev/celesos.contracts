...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, eosio_msig_tester ) try {
   ...
   // required to set up the link between (eosio active) and (celes.prods active)
   //
   //                  eosio active
   //                       |
   ...
   set_authority(N(eosio), "active", authority(1,
      ...
      vector<permission_level_weight>{{{N(celes.prods), config::active_name}, 1}}), "owner",
   ...
   set_code( N(celes.token), contracts::token_wasm() );
   set_abi( N(celes.token), contracts::token_abi().data() );

   create_currency( N(celes.token), config::system_account_name, core_sym::from_string("10000000000.0000") );
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, eosio_msig_tester ) try {
   ...
   // set up the link between (eosio active) and (celes.prods active)
   set_authority(N(eosio), "active", authority(1,
      vector<key_weight>{{get_private_key("eosio", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(celes.prods), config::active_name}, 1}}), "owner",
      { { N(eosio), "active" } }, { get_private_key( N(eosio), "active" ) });
...
BOOST_FIXTURE_TEST_CASE( approve_execute_old, eosio_msig_tester ) try {
   set_code( N(celes.msig), contracts::util::msig_wasm_old() );
   set_abi( N(celes.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();

   //propose with old version of celes.msig
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
   push_action( N(alice), N(propose), mvo()
                  ("proposer",      "alice")
                  ("proposal_name", "first")
