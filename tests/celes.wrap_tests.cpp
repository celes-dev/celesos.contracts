...
public:
   ...
   eosio_wrap_tester() {
      ...
      set_code( N(celes.msig), contracts::msig_wasm() );
      set_abi( N(celes.msig), contracts::msig_abi().data() );
      ...
      set_code( N(celes.wrap), contracts::wrap_wasm(), &system_private_key );
      set_abi( N(celes.wrap), contracts::wrap_abi().data(), &system_private_key );
      ...
      const auto& accnt = control->db().get<account_object,by_name>( N(celes.wrap) );
      abi_def abi;
...
BOOST_FIXTURE_TEST_CASE( wrap_exec_direct, eosio_wrap_tester ) try {
...

   {
      ...
      wrap_trx.actions.emplace_back( get_action( N(celes.wrap), N(exec),
                                                 {{N(alice), config::active_name}, {N(celes.wrap), config::active_name}},
                                                 mvo()
                                                   ("executer", "alice")
                                                   ("trx", trx)
      ) );
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig, eosio_wrap_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( "celes.wrap", name{traces[0]->action_traces[0].act.account} );
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig_unapprove, eosio_wrap_tester ) try {
   ...
   // The proposal should not have sufficient approvals to pass the authorization checks of celes.wrap::exec.
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig_producers_change, eosio_wrap_tester ) try {
   ...
   // The proposal has four of the five requested approvals but they are not sufficient to satisfy the authorization checks of celes.wrap::exec.
   BOOST_REQUIRE_EXCEPTION( push_action( N(celes.msig), N(exec), N(alice), mvo()
                                          ("proposer",      "carol")
                                          ("proposal_name", "first")
                                          ("executer",      "alice")
